...

{
    ...
    has _sa => ( is => 'rw', isa => InstanceOf[ 'FOEGCL::GOTV::StreetAddress' ] );
    ...
    sub _test_method_clean {
        ...
        foreach my $question (keys %qa) {
            eq_or_diff($self->_sa->clean($question), $qa{$question}, 'cleans ' . $question);
        }
    }
    ...
    sub _test_method_standardize {
        ...
        foreach my $question (keys %qa) {
            eq_or_diff($self->_sa->standardize($question), uc $qa{$question}, 'standardizes ' . $question);
        }
    }
}
