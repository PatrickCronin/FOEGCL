...

{
    ...
    sub _test_attr_columns {
        ...
        SKIP: {
            ...
            my $csv = $self->_module_under_test->new(%default_args_custom_columns );
            skip ("because next_record is required for testing columns") if
                ! $csv->can('next_record');
    ...
    sub _test_attr_skip_header {
        ...
        SKIP: {
            ...
            my $csv = $self->_module_under_test->new( %default_args_custom_skip_header );

            skip("because next_record is required for testing skip_header") if
                ! $csv->can('next_record');
            ...
            $csv = $self->_module_under_test->new( %default_args_custom_skip_header );
            $line_count = 0;
            while ($csv->next_record) {
                $line_count++;
            }
